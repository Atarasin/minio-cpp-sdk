...
public:
    ...
    virtual const string &response_body() const override { return data_; }

    virtual int state_code() const override { return state_code_; }
    ...
    virtual HttpClient *add_param(const string &name,
    ...
    static size_t write_bytes(void *ptr, size_t size, size_t count,
                              void *stream) {
    ...
    static size_t read_bytes(void *ptr, size_t size, size_t count,
                             void *userdata) {
    ...
    response_header(const std::string &name) const override {
    ...
    virtual const std::string &error_message() const override { return error_; }
    ...
    bool query() {
        ...
        for (auto &k : params_) {
            curl_formadd(&formpost, (curl_httppost **)&lastptr,
                         CURLFORM_COPYNAME, k.first.c_str(),
                         CURLFORM_COPYCONTENTS, k.second.c_str(), CURLFORM_END);
        }
        ...
        if (type_ == QueryType_PostBody) {
            ...
            stream_put_body.pdata =
                static_cast<const unsigned char *>(body_.pdata);
        ...
        if (res != CURLE_OK) {
            error_ = iLogger::format("Curl error, code is %d, %s", res,
        ...
        // do curl query
        {
            ...
            if (line_position == -1) {
                error_ = iLogger::format(
                    "Worng http response, no line token, code: %d\n",
                    state_code_);
                goto err;
            }
            ...
            response_header_lines_ =
                iLogger::split_string(response_header_string_, "\r\n");
