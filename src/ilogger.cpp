...
const char *level_string(int level) {
    switch (level) {
    case ILOGGER_VERBOSE:
        return "verbo";
    case ILOGGER_INFO:
        return "info";
    case ILOGGER_WARNING:
        return "warn";
    case ILOGGER_ERROR:
        return "error";
    case ILOGGER_FATAL:
...
std::tuple<uint8_t, uint8_t, uint8_t> hsv2rgb(float h, float s, float v) {
    ...
    return make_tuple(static_cast<uint8_t>(r * 255),
...
static struct Logger {
    ...
    void write(const string &line) {
        ...
        if (!keep_run_) {
            ...
            keep_run_ = true;
            flush_thread_.reset(
                new thread(std::bind(&Logger::flush_job, this)));
        }
    ...
    void flush() {
        ...
        if (!local_.empty() && !logger_directory.empty()) {
            ...
            auto now = date_now();
            auto file =
                format("%s%s.txt", logger_directory.c_str(), now.c_str());
            if (!exists(file)) {
    ...
    void set_logger_level(int level) { logger_level = level; }
...
void set_log_level(int level) { __g_logger.set_logger_level(level); }
...
void __log_func(const char *file, int line, int level, const char *fmt, ...) {
    ...
    if (level == ILOGGER_FATAL || level == ILOGGER_ERROR) {
        ...
        n += snprintf(buffer + n, sizeof(buffer) - n, "[\033[32m%s\033[0m]",
                      level_string(level));
    }
#endif
    ...
    while (!ps.empty()) {
        ...
        if (hFind != INVALID_HANDLE_VALUE) {
            do {
                ...
                if (!findDirectory && (find_data.dwFileAttributes &
                                       FILE_ATTRIBUTE_DIRECTORY) !=
                                          FILE_ATTRIBUTE_DIRECTORY ||
                    findDirectory && (find_data.dwFileAttributes &
                                      FILE_ATTRIBUTE_DIRECTORY) ==
                                         FILE_ATTRIBUTE_DIRECTORY) {
    ...
    while (!ps.empty()) {
        ...
        if (handle != 0) {
            while (fileinfo = readdir(handle)) {
                ...
                if (lstat((search_path + fileinfo->d_name).c_str(),
                          &file_stat) < 0)
                    continue;
